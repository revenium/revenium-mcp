...
class TestOnboardingFlowIntegration:
            ...
            with patch('src.revenium_mcp_server.onboarding.detection_service.get_cache_info', return_value=mock_cache_info):
                with patch('src.revenium_mcp_server.onboarding.detection_service.load_cached_config', return_value={'data': 'exists'}):
                    ...
                    with patch.object(conditional_registry, 'register_tools_conditionally', 
                        ...
                        assert init_result['status'] == 'success'
                        # Returning users should have fewer tools registered
                        registered_count = sum(1 for registered in init_result['tools_registered'].values() if registered)
                        assert registered_count >= 1  # At least configuration_status
        ...
        with patch.dict(os.environ, test_env, clear=False):
            with patch('httpx.AsyncClient') as mock_client:
                ...
                with patch('src.revenium_mcp_server.onboarding.env_validation.get_config_value') as mock_get_config:
                    ...
                    with patch('src.revenium_mcp_server.onboarding.env_validation.AuthConfig') as mock_auth:
                        mock_config = Mock()
                        mock_config.team_id = 'test_team'
                        mock_config.api_key = 'test_api_key_12345'
                        mock_auth.return_value.load_from_env.return_value = mock_config
                        ...
                        assert validation_result.api_connectivity['status'] == 'success'
    ...
    async def test_conditional_tool_registration_flow(self):
        ...
        with patch('src.revenium_mcp_server.onboarding.conditional_registration.get_onboarding_state') as mock_state:
            ...
            with patch.object(conditional_registry, '_register_single_tool', return_value=True):
                ...
                # Complete setup users should have minimal tools registered
                assert registration_result['configuration_status'] is True
                assert registration_result['welcome_and_setup'] is False
                assert registration_result['setup_checklist'] is False
                assert registration_result['verify_email_setup'] is False
    ...
    async def test_setup_completion_progression(self):
        ...
        with patch('src.revenium_mcp_server.onboarding.detection_service.get_config_value') as mock_get_config:
            ...
            # Mock the onboarding state to reflect API key configuration
            with patch('src.revenium_mcp_server.onboarding.detection_service.get_onboarding_state') as mock_state:
                mock_state.return_value = Mock(
                    setup_completion={'api_key_configured': True, 'team_id_configured': False, 'email_configured': False}
                )
            ...
            with patch('src.revenium_mcp_server.onboarding.detection_service.get_onboarding_state') as mock_state:
                ...
                with patch('src.revenium_mcp_server.onboarding.env_validation.validate_environment_variables') as mock_validate:
                    mock_validate.return_value = Mock(summary={'overall_status': True})
    ...
    async def test_onboarding_refresh_flow(self):
        ...
        with patch.object(conditional_registry, 'should_refresh_registration', return_value=False):
            refresh_result = await refresh_onboarding()
            
            assert refresh_result['status'] == 'no_refresh_needed'
            assert refresh_result['smart_defaults_updated'] is False
